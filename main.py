...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://blogz:test123@localhost:8889/blogz'
app.config['SQLALCHEMY_ECHO'] = True

db = SQLAlchemy(app)
app.secret_key = 'zy27kGcys&pztq'
...
@app.before_request
def require_login():
    allowed_routes = ['login','signup', 'index', 'logout', 'all_blog']
...
def all_blog():
    ...
    if "id" in request.args:
        ...
        title = ""
        return render_template('singleUser.html', title= title, blog = blog, post_id = post_id)

    elif "user" in request.args:
        user_id = request.args.get('user')
        blog = Blog.query.filter_by(owner_id = user_id).all()
        return render_template('singleUser.html', title = title, blog = blog)
